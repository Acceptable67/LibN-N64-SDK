...
[[maybe_unused]] static int *local_buffer =
    reinterpret_cast<int *>(FRAMEBUFFER_ADDR);
...
void DrawCircle(
    LibPos pos, u32 scale, const u32 color, bool isFilled, float cStepSize) {
    if (isFilled) {
        for (float scaler = 0; scaler <= scale; scaler += 0.3) {
            for (float angles = 0; angles < 25 * scaler; angles += cStepSize) {
                DrawPixel(
                    {static_cast<u32>(pos.x + cosf(angles) * 3.1415f * scaler),
                        static_cast<u32>(
                            pos.y + sinf(angles) * 3.1415f * scaler)},
                    color);
            }
        }
    } else {
        for (float angles = 0; angles < 25 * scale; angles += cStepSize) {
            DrawPixel(
                {static_cast<u32>(pos.x + cosf(angles) * 3.1415f * scale),
    ...
    for (u32 row = 0; row < font_width; row++) {
        ...
        for (u32 col = 0; col < font_height; col++) {
            if (trans) {
                if (c & 0x80) {
                    DrawPixel(
                        {pos.x + col, pos.y + row}, localColor.Foreground);
                }
            } else {
...
void Send() {
    ...
    /*I have no idea why this works with + 285. It should just queue the list
    then execute, but it doesn't work that way. It needs a very high 'magic'
    number to get the screen refresh perfect.*/
}

void DebugAddr() {
    printf(
        "%08X %08X", (u32)cBuffer.data(), (u32)cBuffer.data() + cBuffer.size());
