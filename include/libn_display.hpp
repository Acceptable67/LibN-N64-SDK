...
inline constexpr uint32_t MakeColor(u32 r, u32 g, u32 b, u32 a) {
    return (r << 24) | (((g & 0x00FFFFFF) << 16)) | (((b & 0xFF00FFFF) << 8)) |
           ((a & 0xFFFF00FF));
}
...
void DrawLine(LibPos, LibPos, const u32);        /*add implementation*/
void DrawTri(LibPos, LibPos, LibPos, const u32); /*add implementation*/
...
requires(!std::is_member_function_pointer<T>::value) ||
    (!std::is_member_function_pointer<Args...>::value) void DrawTextFormat(
        const LibPos, const std::string format, T arg, Args... args);
